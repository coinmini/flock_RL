...
class RLInputData(BaseInputData):
    ...
    max_params: int
    local_model_path: str = None  # 本地模型文件路径，如果提供则跳过 HuggingFace 下载
...
class RLValidationModule(BaseValidationModule):
    ...
    def _load_model(self, repo_id: str, filename: str = "model.onnx", revision: str = "main", max_params: int = None, local_model_path: str = None):
        """Download and load ONNX model from HuggingFace Hub or local path"""
        if local_model_path:
            # 使用本地模型文件
            import os
